...
impl KbGameEngine for ExampleGame {
	...
	fn initialize_world(&mut self)
	{
		...
		let mut x = -1.9;
		while x < 1.9 {
			let tree_width = game_random_f32!(0.08, 0.15);
			let tree_height = game_random_f32!(0.16, 0.2);
			self.game_objects.push(GameObject { 
...
fn main() {
    ...
    let run_game = kb_engine3::run_game::<ExampleGame>(game_config);
